...
class ChildController extends AbstractController
{
    ...
    {
        $child = new Child();
        //si je suis admin je fais apparaitre un formaulaire quie contient
        //le champ pour affecter un parent a un enfant
        if (in_array('ROLE_ADMIN', $this->getUser()->getRoles())) {
            $form = $this->createForm(ChildAdminType::class, $child);
        } else {
            //si je suis parent, le formulaire n'as pas le champ parent
            $form = $this->createForm(ChildType::class, $child);
        }
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            //on enregistre le parent (User) sur l'objet enfant
            $child->setUser($this->getUser());
            //ouis on sauvegarde l'objet en BDD
